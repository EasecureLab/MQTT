...
//        datamsg = "5A450201201100000000A7E70000000080000304000000027029E416";
        ...
        // 生成获取电表数据的数据包
        datamsg = Highway1Support2.AmmeterfromConnecttoMsg20Current(connectToMsg);
        // 获取返回的电表数据
        String fromAmmeter= Highway1Client2.Ammetergetbackmsg(socket,datamsg);

        System.out.println("开始采集数据！");

        System.out.println("第"+count+"次："+"设备ID："+ Highway1Support2.AmmetergetEquipmentID(fromAmmeter)+"——总线ID："+ Highway1Support.AmmetergetHighwayID(Integer.toString(port))
                +"——时间戳："+ Highway1Support2.AmmetergetTime(fromAmmeter)+"——数据信息："
                +"湿度："+ Highway1Support2.AmmetergetData(fromAmmeter)[0]/10+"%"
                + " 温度："+Highway1Support2.AmmetergetData(fromAmmeter)[1]/10+"℃"+"——设备标签："+ Highway1Support.getTag());
        ...
        while (true) {
            try {
                ...
                datamsg = Highway1Support2.AmmeterfromConnecttoMsg20Current(fromAmmeter);
                fromAmmeter = Highway1Client2.Ammetergetbackmsg(socket, datamsg);
